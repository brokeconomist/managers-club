...
def show_discount_proposal_ui():
    ...
    cash_discount_rate = st.number_input("Έκπτωση για Πληρωμή τοις Μετρητοίς (%)", value=2.0, step=0.1) / 100
    pct_sales_with_discount = st.number_input("Ποσοστό Πωλήσεων με Έκπτωση (%)", value=40.0, step=1.0) / 100
    days_collection_discounted = st.number_input("Μέρες Είσπραξης Πωλήσεων με Έκπτωση", value=30)
    pct_sales_without_discount = st.number_input("Ποσοστό Πωλήσεων χωρίς Έκπτωση (%)", value=60.0, step=1.0) / 100
    ...
    pct_sales_with_discount_after_increase = st.number_input("Ποσοστό Πωλήσεων με Έκπτωση μετά την Αύξηση (%)", value=70.0, step=1.0) / 100
    pct_sales_without_discount_after_increase = st.number_input("Ποσοστό Πωλήσεων χωρίς Έκπτωση μετά την Αύξηση (%)", value=30.0, step=1.0) / 100
    pct_current_bad_debts = st.number_input("% Τρεχουσών Επισφαλειών", value=1.0, step=0.1) / 100
    pct_bad_debt_reduction_after_discount = st.number_input("% Μείωσης Επισφαλειών λόγω Πληρωμών μετρητοίς", value=0.5, step=0.1) / 100
    cost_of_capital = st.number_input("Κόστος Κεφαλαίου (%)", value=20.0, step=0.1) / 100
