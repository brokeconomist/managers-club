...
def show_discount_cash_tool():
    ...
    with st.form("discount_form"):
        ...
        with col1:
            sales_now = st.number_input("Τρέχουσες Πωλήσεις (€)", value=DEFAULTS["current_sales"], min_value=0, step=100, format="%d")
            extra_sales = st.number_input("Επιπλέον Πωλήσεις λόγω Έκπτωσης (€)", value=DEFAULTS["extra_sales"], min_value=0, step=50, format="%d")
        ...
        with col2:
            days_discount = st.number_input("Μέρες για Πληρωμή με Έκπτωση", value=DEFAULTS["cash_discount_days"], min_value=0, max_value=180, step=1, format="%d")
            days_accept = st.number_input("Μέρες Πληρωμής όσων Αποδέχονται την Έκπτωση", value=DEFAULTS["cash_discount_accept_days"], min_value=0, max_value=180, step=1, format="%d")
            days_non_accept = st.number_input("Μέρες Πληρωμής όσων Δεν Αποδέχονται την Έκπτωση", value=DEFAULTS["non_discount_accept_days"], min_value=0, max_value=180, step=1, format="%d")
            wacc = st.slider("Κόστος Κεφαλαίου (WACC %)", 0.0, 30.0, float(DEFAULTS["wacc"]*100), step=1.0) / 100
            avg_collection_days = st.number_input("Τρέχουσα Μέση Περίοδος Είσπραξης (μέρες)", value=DEFAULTS["current_collection_period"], min_value=0, max_value=365, step=1, format="%d")
        ...
        if submitted:
            ...
            - 📉 Πέρα από το βέλτιστο σημείο, το κόστος της έκπτωσης αυξάνεται περισσότερο από το όφελος.
            """)
