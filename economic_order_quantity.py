...
def show_economic_order_quantity():
    ...
    st.write(f"**ΣΥΝΟΛΙΚΟ ΚΟΣΤΟΣ ΠΡΟΜΕΙΘΕΙΩΝ ΜΙΑΣ ΠΕΡΙΟΔΟΥ Κ:** {format_number_gr(total_procurement_cost, 0)}")
    st.write(f"**ΣΤΑΘΕΡΟ ΚΟΣΤΟΣ ΠΡΟΜΕΙΘΕΙΩΝ ΜΙΑΣ ΠΕΡΙΟΔΟΥ ΚF:** {format_number_gr(fixed_procurement_cost_per_period, 0)}")
    ...
    st.write(f"**ΤΟΚΟΣ ΣΕ % i:** {format_percentage(monthly_interest_rate)}")
